...
int P_WF_scheduler (){
    ...
    {
        if (threadArray[Thread_index]->STATE == READY)
        {
            ready_array[count_ready] = Thread_index;
            count_ready++;
        }
        
    }
    srand(time(NULL));
    array_index = rand() % count_ready;
    return ready_array[array_index];
}
void runThread(int signum)
{
    ...
    Thread_index = P_WF_scheduler();//determine the Thread_index
